...
public class ServletBuscarProducto extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try ( PrintWriter out = response.getWriter()) {
            String nombre = request.getParameter("obtenerNombre");
            
            ControladorProducto cont = new ControladorProducto();
            Producto prod = cont.BuscarProducto(nombre);
            if (prod.getNombre() != null && prod.getNombre() != null && prod.getPrecio()!= null && prod.getStock()!=null){
                request.setAttribute("producto", prod);
                request.getRequestDispatcher("BuscarProducto.jsp").forward(request, response);
            }else{
                response.sendRedirect("errorBuscar.jsp");
            }
